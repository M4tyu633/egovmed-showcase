...
export default function Symptom({ c, lang, S, A }) {
  ...
  return (
    <div className="screen">
      ...
      <p className="sub" data-stagger>{DEMO_MODE ? 'Choose a sample symptom below, or type a fictional example to explore the routing.' : c.symptomHint}</p>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, margin: '22px 0' }}>
        <button
          onClick={A.toggleRec}
          aria-label={DEMO_MODE ? 'Use a sample symptom' : S.recording ? c.micStop : c.micTap}
        ...
        <span style={{ fontSize: '0.85em', fontWeight: 600, color: S.recording ? 'var(--red)' : 'var(--muted)' }}>{DEMO_MODE ? 'Use a sample symptom' : S.recording ? c.micStop : c.micTap}</span>
      </div>
